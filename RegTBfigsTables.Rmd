...
p2 <- ggplot(subset(age, year==yr), aes(age, new_sp_100k, fill=sex)) + geom_bar(stat="identity", position="dodge") + facet_wrap(~sh.name, scales='free_y', ncol=2) + theme_few() + scale_fill_brewer(type="qual", palette=3) + xlab("Age group")
...
p1 <- ggplot(age, aes(year, new_sp_100k, color=age)) + geom_line() + facet_grid(sh.name~sex, scales='free_y') + theme_few() + scale_x_continuous("", breaks = pretty_breaks()) + scale_color_brewer(type="seq", palette=4)
...
```{r t.notif, echo=FALSE, results='asis', warning=FALSE}
