...
```{r f.notif.trend, echo=FALSE, fig.width=7, height=5}
...
ggplot(notc, aes(year, value, color=variable, ymin=0)) + geom_line(size=1) + theme_few() + scale_x_continuous("", breaks = pretty_breaks()) + scale_y_continuous("TB cases per 100 000", breaks = pretty_breaks()) + theme(legend.position="none") + geom_text(data=subset(notc, year==max(notc$year)), aes(label=variable), hjust=1.1, vjust=3) + scale_color_brewer(type="qual", palette=6)
...
p2 <- ggplot(subset(age, year==yr), aes(age, new_sp_100k, fill=sex)) + geom_bar(stat="identity", position="dodge") + facet_wrap(~sh.name, scales='free_y', ncol=2)   + theme_few() + scale_fill_brewer('Sex', type="qual", palette=6) + xlab("Age group") 
...
``` {r f.txout.bar, echo=FALSE, fig.width=7}
...
ggplot(trc, aes(year, value, fill=variable)) + geom_bar(stat="identity", position="stack") + theme_few() + scale_fill_brewer('Outcome', type="qual", palette=6) + xlab("") + scale_y_continuous("Percent of cohort") + coord_cartesian(ylim=c(60,100))

trd <- trc
trd$type <- ifelse(trd$variable=="Success", "Favourable", "Unfavourable")

ggplot(trd, aes(year, value, color=variable)) + geom_line(size=1, ) + facet_wrap(~type, scales="free_y", ncol=1) + theme_few() + scale_fill_brewer('Outcome', type="qual", palette=6) + scale_x_continuous("", breaks = pretty_breaks()) + scale_y_continuous("Percent of cohort", expand=c(0.1,0)) 

ggplot(trd[trd$year > 2006, ], aes(year, value, color=variable)) + geom_line(size=1, ) + facet_wrap(~type, scales="free_y", ncol=1) + theme_few() + scale_fill_brewer('Outcome', type="qual", palette=6) + scale_x_continuous("", breaks = pretty_breaks()) + scale_y_continuous("Percent of cohort", expand=c(0.6,0)) 
...
<a id="f.est.data"></a> 
#### `r I(pasteLabel("Figure", figCount, "f.est.data", insLink=FALSE))` Treatment outcomes for new smear-positive cases in the Region, 2000--`r yr-1`.
``` {r f.est.data, echo=FALSE, fig.width=7, fig.height=4}
...
esth$var <- factor(esth$var, levels=c("inc", "prev", "mort_exc_tbhiv"), labels=c("Incidence", "Prevalence", "Mortality (excluding HIV)"))

ggplot(esth, aes(year, best, fill=var, ymin=0)) + geom_line(size=1, aes(color=var)) + geom_ribbon(aes (year, best, ymin=lo, ymax=hi), alpha=0.2) + theme_report() + facet_wrap(~var, scales="free_y") + theme(legend.position="none") + scale_x_continuous("", minor_breaks=seq(1990, yr, 1)) + scale_y_continuous(breaks=pretty_breaks()) + ylab("Rate per 100 000 population")
