...
In `r yr`, countries and areas in the Region reported `r n1` people with TB disease (`r I(pasteLabel("Table", tableCount, "t-notif"))`) making up `r n2`% of the global burden. Of these cases, `r n3`% (`r n4`) were new episodes of TB disease (either new or relapse cases). Within the Region China accounts for `r n5`% (`r n6`) of the caseload with the Philippines and Viet Nam following with `r n7`% (`r n8`) and `r n9`% (`r n10`), respectively. TB notification rates, expressed as cases per 100 000 population, vary substantially in the Region with the highest rates (`r "\u2265"` 200) found in `r n12`  with `r n13` per 100 000 population, respectively (`r I(pasteLabel("Figure", figCount, "m-notif"))`). 
...
notb$`New laboratory-confirmed` <- ifelse(notb$year < 2013, notb$new_sp / notb$e_pop_num * 1e5, notb$new_labconf / notb$e_pop_num * 1e5)
notc <- melt(notb[c("year", "New laboratory-confirmed", "All forms of TB")], id=1)
...
Between 2002 and 2007, case notification rates in the Region increased from 47 to 77 per 100 000 population in all forms of TB and from 22 to 37 per 100 000 population in new laboratory-confirmed TB cases. After 2005 the case notification rates for all forms of TB stabilized, and new laboratory-confirmed cases have started to decrease (`r I(pasteLabel("Figure", figCount, "f-notif-trend"))`). 

--------------------------------------------------------

<a id="f-notif-trend"></a> 
**`r I(pasteLabel("Figure", figCount, "f-notif-trend", insLink=FALSE))` Tuberculosis case notification rate (all forms and new laboratory-confirmed) per 100 000 population in the Western Pacific Region, `r min(notc$year)`--`r max(notc$year)`**  
...
# For paragraph
n15 <- age %>% group_by(area, sex) %>% summarize(newrel=sum0(newrel)) %>% spread(sex, newrel) %>% mutate(sr=signif(Male/Female,2)) %>% arrange(desc(sr)) %>% head(1) 

```

`r I(pasteLabel("Figure", figCount, "f-agesex-bar"))` shows age group and sex-specific case notification rates of new laboratory-confirmed cases for countries with a high burden of TB in the Region with available data (note that the scale of the vertical axis is different for each country). Many countries follow a typical pattern for cross-sectional observations with increasing notification rates towards older populations except in Mongolia. In general, males are more affected than females with male-to-female TB ratios as high as `r n15$sr` as seen in `r n15$area`.
...
`r I(pasteLabel("Figure", figCount, "f-agesex"))` shows trends of notification rates of new laboratory-confirmed cases of age- and sex-specific groups in the seven countries with a high-burden of TB from 2000 to `r yr`. Some countries demonstrated a declining trend of case notification for almost all age- and sex- groups, such as Cambodia and Viet Nam while others showed a less apparent trend. These time trends are affected by the revision of the TB notification framework (see Box 1).

--------------------------------------------------------

<a id="f-agesex"></a> 
**`r I(pasteLabel("Figure", figCount, "f-agesex", insLink=FALSE))` Trend of age group- and sex-specific notification rates (per 100 000 population) of new and relapse^a^ tuberculosis cases in seven countries with a high-burden of TB in the Western Pacific Region, `r min(age$year)`--`r max(age$year)`**  
``` {r f-agesex, warning=FALSE, fig.height=15}
if (!figtabs.at.end) {

p1 <- ggplot(age, aes(year, newrel_100k, color=age)) + geom_line() + facet_grid(sh.name~sex, scales='free_y') + theme_report() + scale_x_continuous("", breaks = pretty_breaks()) + scale_color_brewer("Age group", type="seq", palette=4) + guides(color = guide_legend(reverse = TRUE))
# Note that 'free_y' only affects scales between countries, not between sexes in countries.

p1b <- p1 + scale_y_continuous("New and relapse cases per 100 000")
# p1 + scale_y_log10("New laboratory-confirmed cases per 100 000") 

p1b

ggsave(p1b, file=paste0(pasteLabel("./figure/figure", figCount, "f-agesex", insLink=FALSE, sepper=""), ".pdf"), height = 15)
...
**Box 1 Effect of new case definitions on age and sex disaggregation**

> Prior to 2013, age and sex disaggregated notification data was further 
> disaggregated by site of the disease (pulmonary or extrapulmonary) and by 
> smear status (smear-positive and smear-negative/unknown/not done) for 
> pulmonary cases. These data were only collected for new patients. Since the 
> introduction of the revised recording and reporting framework, this further 
> disaggregation is no longer collected in favour of one age and sex table 
> with relapse cases included. This affects the time trend of notifications and also 
> increases uncertainty around diagnosed cases since clinically-diagnosed cases 
> may not be actual TB cases.
...
n34 <- tb %>% mutate(idef=signif((c_newinc-newrel_coh)/c_newinc * 100, 2)) %>% filter(year==yr-1, iso3 %in% whbc, idef > 2) %>% select(country, idef) %>% arrange(desc(idef))
...
Treatment success in the Region continues to be over 85% (`r I(pasteLabel("Figure", figCount, "f-txout-line"))`), a trend observed since  1997. Across the Region, `r n31` countries and areas reached or maintained 85% treatment success. Among the countries with a high burden of TB, treatment success was highest in `r n32[1,1]` (`r n32[1,3]`%) followed by `r n32[2,1]` (`r n32[2,3]`%), `r n32[3,1]` (`r n32[3,3]`%), `r n32[4,1]` (`r n32[4,3]`%), `r n32[5,1]` (`r n32[5,3]`%) and `r n32[6,1]` (`r n32[6,3]`%). The treatment success rate of `r n32[7,1]` was the lowest at `r n32[7,3]`% with `r n33` of the `r yr-1` cohort either lost to follow-up or un-evaluated. The proportion of notified cases who were not reported as being put on treatment was over two percent in `r n35` countries, namely `r n35c` with `r n35n` percent, respectively. 

--------------------------------------------------------

<a id="f-txout-line"></a> 
**`r I(pasteLabel("Figure", figCount, "f-txout-line", insLink=FALSE))` Trend of treatment success among new laboratory-confirmed cases in select countries of the Western Pacific Region, `r min(trd$year)`--`r max(trd$year)`**  
...
tha1 <- subset(tb, year %in% 2005:yr & g_whoregion=="WPR" & iso3 %in% whbc, select=c("country", "year", "iso3", "g_hbc22", "c_notified", "hivtest", "hivtest_pos", "hiv_cpt", "hiv_art", "hivtest_pct_numerator", "hivtest_pct_denominator", "hivtest_pos_pct_numerator", "hivtest_pos_pct_denominator", "hiv_cpt_pct_numerator", "hiv_cpt_pct_denominator", "hiv_art_pct_numerator", "hiv_art_pct_denominator"))
...
<a id="f-tbhiv"></a> 
**`r I(pasteLabel("Figure", figCount, "f-tbhiv", insLink=FALSE))` Progress in TB/HIV activities in seven countries in the Western Pacific Region with a high burden of TB, `r min(thb$year)`--`r max(thb$year)`**  
...
`r if(figtabs.at.end) "Tables and Figures" `
--------------------------------------------------------

**`r I(pasteLabel("Table", tableCount, "t-notif", insLink=FALSE))`. Tuberculosis case notification from countries and areas of the Western Pacific Region, `r yr`** 

```{r t-notif2, results='asis'}
if (figtabs.at.end) {
...
**`r **  
```{r m-notif2, fig.height=5}
if (figtabs.at.end) {
paste(I(pasteLabel("Figure", figCount, "m-notif", insLink=FALSE)), "Tuberculosis case notification rate (new and relapse) per 100 000 population in countries and areas of the Western Pacific Region, ", yr)
...
**`r I(pasteLabel("Figure", figCount, "f-agesex-bar", insLink=FALSE))` Age group- and sex-specific notification rates (per 100 000 population) of new and relapse TB cases in countries with a high-burden of TB in the Western Pacific Region, `r yr`**  
``` {r f-agesex-bar2, fig.height=11, warning=FALSE}
if (figtabs.at.end) {
p3
}
```
...
```{r Endcopy, include=FALSE, echo=FALSE}
