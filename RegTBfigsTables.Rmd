...
### Results
Through routine surveillance, countries and areas in the Region reported `r a2` million TB cases in `r yr`. The case notification rate increased in the early 2000s, appears to have stabilized in recent years and is in decline for all forms and laboratory-confirmed cases. The age and sex breakdown for laboratory-confirmed TB case rates by country shows generally higher rates with increased age and declining rates over time for all age groups. Treatment success remains high in the Region with `r a3` countries reaching or maintaining an 85% success rate. HIV testing among TB patients has increased gradually along with a slow decline in the number of HIV-positive patients found.
...
Case definitions for TB were revised in 2013[@WorldHealthOrganization2013] and cases previously known as smear-positive are now included in the category of laboratory-confirmed cases which includes cases confirmed by any laboratory method. Similarly, smear-negative cases are combined with all clinically diagnosed cases including where smear was not done or results are unknown. This obscures the number of cases where laboratory confirmation was sought by including them with the number of cases for which confirmation was not attempted. This can be a good indicator of uncertainty around the number of notified cases. Between 2008 and 2012 at the regional level, a maximum of `r m6`% of cases did not have a smear result in any given year. However at the country level, the proportion was as high as `r m5` percent  in `r m4`, respectively.
...
# for paragraph
n16a <- notc %>% filter(variable=="New laboratory-confirmed", year %in% c(2005,yr)) %>% arrange(year) %>% select(year, value)
n16 <- abs(signif(a.rate(start=n16a[1,2], end=n16a[2,2], years=n16a[2,1]-n16a[1,1]) * 100, 2)) 

```

Between 2002 and 2005, case notification rates in the Region of all forms of TB increased from 47 to 74 cases per 100 000 population per year and have stabilized since then at 73--78 cases per 100 000 population per year. New laboratory-confirmed cases peaked in 2005 at 39 cases per 100 000 population per year (up from 22 in 2002), and has declined continuously to `r yr` at an average rate of `r n16`% per year (`r I(pasteLabel("Figure", figCount, "f-notif-trend"))`). 
...
`r I(pasteLabel("Figure", figCount, "f-agesex"))` shows trends of notification rates of new laboratory-confirmed cases of age- and sex-specific groups in the seven countries with a high-burden of TB from 2000 to `r yr`. Cambodia and Viet Nam demonstrate a declining trend for almost all age- and sex- groups.   while others showed a less apparent trend. These time trends are affected by the revision of the TB notification framework (see Box 1).
...
``` {r f-agesex, warning=FALSE, fig.height=5, fig.width=4}

age.iso <- data.frame(a=unique(age$iso3)[1:2], b=unique(age$iso3)[3:4], c=unique(age$iso3)[5:6], d=unique(age$iso3)[7:8]) 

for(bit in names(age.iso)){
  p1 <- age %>% filter(iso3 %in% age.iso[[bit]]) %>% ggplot(aes(year, newrel_100k, color=age)) + geom_line() + facet_grid(sh.name~sex, scales='free_y') + theme_report() + scale_x_continuous("", breaks = pretty_breaks()) + scale_color_brewer("Age group", type="div", palette="RdYlBu") + guides(color = guide_legend(reverse = TRUE)) + scale_y_log10("New and relapse cases per 100 000 (log scale)") 
...
``` {r f-agesex-a, warning=FALSE, fig.height=5, fig.width=4}
age.a
```

``` {r f-agesex-b, warning=FALSE, fig.height=5, fig.width=4}
age.b
```

``` {r f-agesex-c, warning=FALSE, fig.height=5, fig.width=4}
age.c
```

``` {r f-agesex-d, warning=FALSE, fig.height=5, fig.width=4}
age.d
```
...
Prior to 2013, age and sex disaggregated notification data was further disaggregated by site of the disease (pulmonary or extrapulmonary) and by smear status (smear-positive and smear-negative/unknown/not done) for pulmonary cases. These data were only collected for new patients. Since the introduction of the revised recording and reporting framework, this further disaggregation is no longer collected in favour of one age and sex table with relapse cases included. This affects the time trend of notifications and also increases uncertainty around diagnosed cases since clinically-diagnosed cases may not be actual TB cases.
