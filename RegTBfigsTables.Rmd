...
# Notification table
tbb <- subset(n, year==yr & g_whoregion=="WPR", select=c('country', 'g_whoregion', 'g_hbc22', 'c_notified', "new_sp", "new_sn", "new_su", "new_ep", "new_oth", "ret_rel", 'c_newinc', "c_ret", "newret_oth", "new_labconf", 'c_new', "e_pop_num"))
...
tbm <- xtable(tbc[c("area", "c_notified", "tot_newrel_100k", "new_sp", "new_sn", "new_su", "new_ep", "new_oth", "new_labconf2", "newpulm_lab_pct", "ret_rel", "ret_nrel", "c_newinc")])
digits(tbm) <- 0
...
write.csv(mc, file=paste0(pasteLabel("D:/Users/hiattt/Dropbox/Code/Surveillance reports/figure data/figure", figCount, "m-notif", insLink=FALSE), ".csv"), row.names=FALSE)
```
...
"<TR> <TD colspan=3></TD> 
  ...
  glue("<TR> <TD colspan=13>Blank cells indicate data not reported. -- indicates values that cannot be calculated.<br>
...
write.csv(notc, file=paste0(pasteLabel("D:/Users/hiattt/Dropbox/Code/Surveillance reports/figure data/figure", figCount, "f-notif-trend", insLink=FALSE), ".csv"), row.names=FALSE)
```
...
write.csv(age, file=paste0(pasteLabel("D:/Users/hiattt/Dropbox/Code/Surveillance reports/figure data/figure", figCount, "f-agesex-bar", insLink=FALSE), ".csv"), row.names=FALSE)

write.csv(age, file=paste0(pasteLabel("D:/Users/hiattt/Dropbox/Code/Surveillance reports/figure data/figure", figCount, "f-agesex", insLink=FALSE), ".csv"), row.names=FALSE)
```
...
write.csv(trc, file=paste0(pasteLabel("D:/Users/hiattt/Dropbox/Code/Surveillance reports/figure data/figure", figCount, "f-txout-bar", insLink=FALSE), ".csv"), row.names=FALSE)
```
...
``` {r f-txout-bar, fig.width=7}

ggplot(trc, aes(year, value, fill=variable)) + geom_bar(stat="identity", position="stack") + theme_report() + scale_fill_brewer('Outcome', type="qual", palette=6) + xlab("") + scale_y_continuous("Percent of cohort") + coord_cartesian(ylim=c(60,100)) + guides(fill = guide_legend(reverse = TRUE))

trd <- trc
trd$type <- ifelse(trd$variable=="Success", "Favourable", "Unfavourable")

ggplot(trd, aes(year, value, color=variable)) + geom_line(size=1, ) + facet_wrap(~type, scales="free_y", ncol=1) + theme_report() + scale_fill_brewer('Outcome', type="qual", palette=6) + scale_x_continuous("", breaks = pretty_breaks()) + scale_y_continuous("Percent of cohort", expand=c(0.1,0)) 

ggplot(trd[trd$year > 2006, ], aes(year, value, color=variable)) + geom_line(size=1, ) + facet_wrap(~type, scales="free_y", ncol=1) + theme_report() + scale_fill_brewer('Outcome', type="qual", palette=6) + scale_x_continuous("", breaks = pretty_breaks()) + scale_y_continuous("Percent of cohort", expand=c(0.6,0)) 
...
tha1 <- subset(tb, year >= 2005 & g_whoregion=="WPR" & iso3 %in% whbc, select=c("country", "year", "iso3", "g_hbc22", "g_hbhiv63", "c_notified", "hivtest", "hivtest_pos", "hiv_cpt", "hiv_art", "hivtest_pct_numerator", "hivtest_pct_denominator", "hivtest_pos_pct_numerator", "hivtest_pos_pct_denominator", "hiv_cpt_pct_numerator", "hiv_cpt_pct_denominator", "hiv_art_pct_numerator", "hiv_art_pct_denominator"))

tha2 <- aggregate(tha1[6:ncol(tha1)], by=list(year=tha1$year), FUN=sum, na.rm=TRUE)
tha2[c("country", "iso3")] <- "WPR"
tha2[c("g_hbc22", "g_hbhiv63")] <- NA
tha <- rbind(tha1, tha2)
...
thb$country <- factor(thb$iso3, c("KHM", "CHN", "LAO", "MNG", "PNG", "PHL", "VNM", "WPR"), c("Cambodia", "China", "Lao PDR", "Mongolia", "PNG", "Philippines", "Viet Nam", "WPR"))

write.csv(thb, file=paste0(pasteLabel("D:/Users/hiattt/Dropbox/Code/Surveillance reports/figure data/figure", figCount, "f-tbhiv", insLink=FALSE), ".csv"), row.names=FALSE)
```
...
esta <- subset(araw, year >=1990 & group_name=="WPR", select=eget)
...
for(var in c("inc", "prev", "mort_exc_tbhiv")){
  df <- subset(estd, select=c("year", glue("e_", var, "_100k"), glue("e_", var, "_100k_lo"), glue("e_", var, "_100k_hi")))
...
write.csv(esth, file=paste0(pasteLabel("D:/Users/hiattt/Dropbox/Code/Surveillance reports/figure data/figure", figCount, "f-est", insLink=FALSE), ".csv"), row.names=FALSE)
```
