...
tbb <- tbl %>% filter(year==yr) %>% mutate(area=country) %>% select(area, g_whoregion, c_notified, c_newinc, new_labconf, new_clindx, new_ep, ret_rel_labconf, ret_rel_clindx, ret_rel_ep, ret_nrel)

# make aggregate rows
tbbr <- aggregate(tbb[3:ncol(tbb)], by=list(area=tbb$g_whoregion), FUN=sum, na.rm=TRUE)

# combine together
tbc <- rbind(tbb[c(1, 3:ncol(tbb))], tbbr) # tbbh, , tbbga
...
tbl %>% filter(year==yr) %>% ggplot(aes(c_newinc_100k, c_newinc, group=iso3, size=e_pop_num/1e6)) + geom_text(aes(label=iso3), size=4, hjust=-.3, vjust=-.5) + geom_point() + theme_report() + labs(x="New and relapse cases per 100 000 (log scale)", y="Total new and relapse cases", size="Population \n(millions)") + scale_x_log10(limits=c(5,100), breaks=c(5, 10, pretty(20:100)))
...
tbl %>% ggplot(aes(year, c_newinc_100k, ymin=0)) + geom_line(size=1) + theme_report() + labs(x="", y="Total new and relapse cases per 100 000") + facet_wrap(~iso3, scales="free_y") + geom_smooth() 
...
p1 + scale_y_continuous("New and relapse cases per 100 000") + geom_vline(aes(xintercept=c(2006, 2000)), color="red", alpha=.1)
...
ggplot(subset(age, year>=1995 & iso3 %in% lic[1:9]), aes(year, newrel_100k, color=age)) + geom_line(size=1) + facet_wrap(~iso3, scales='free_y', nrow=2) + theme_report() + scale_x_continuous("", breaks = pretty_breaks()) + scale_color_brewer("Age group", type="seq", palette=4) + scale_y_continuous("New and relapse cases per 100 000") + guides(color = guide_legend(reverse = TRUE)) + theme(legend.text.align=1)
...
ggplot(obb, aes(year, migtb_pct, ymin=0)) + geom_point(aes(size=mig)) + geom_line() + theme_report() + scale_x_continuous("", limits=c(2007,2013)) + scale_y_continuous("Foreign-born TB cases (% of total notified)") + facet_wrap(~iso3, scales = "free_y") + scale_size("International migrant stock 2010 (% of population)") + theme(legend.position="bottom")
...
ggplot(aes(mig, migtb_pct)) + geom_point(aes(color=iso3)) + geom_line(aes(x,y), data.frame(x=c(0,75), y=c(0,75))) + theme_report() + scale_x_continuous("International migrant stock 2010 (% of population)") + scale_y_continuous("Foreign-born TB cases 2013 (% of total notified)")
  direct.label(obq)
...
ggplot(aes(mig, c_newinc_100k)) + geom_point(aes(color=iso3)) + geom_line(aes(x,y), data.frame(x=c(0,75), y=c(0,75))) + theme_report() + scale_x_continuous("International migrant stock 2010 (% of population)") + scale_y_continuous("New and relapse TB cases per 100 000")
  direct.label(obq)


# Notes:
# 3 groups: AUS and NZL have a high proportion of TB from migrants, but very low migrant stock. So most of their migrants come from HBCs? If the receiving country had the same TB burden as the sending countries, we would expect ... It's TB and not all. The proportion of migrants in the all population is higher than in the TB population
...
ggplot(trd, aes(year, success, color=iso3)) + geom_line(size=1) + theme_report() + geom_text(data=end.labels(trd, "year", "success", min.spread=2.1), aes(label=iso3), hjust=-0.2, vjust=0, size=4)  + scale_y_continuous("Percent of cohort successfully treated") + scale_x_continuous("", limits=c(2000,2012.7)) + coord_cartesian(ylim=c(0,100)) + scale_size_discrete(range = c(1,1.5)) + theme(legend.position="none") + scale_color_brewer(type="qual", palette=6)  + guides(fill = guide_legend(reverse = TRUE))
...
trb$Defaulted <- rowSumsNA(trb[c("new_sp_def", "newrel_lost")]) / trb$coh * 100
...
trc <- melt(trb[c("year", "iso3", "Success", "Died", "Failed", "Defaulted", "Not evaluated")], id=1:2)
...
ggplot(trc, aes(year, value, fill=variable)) + geom_bar(stat="identity", position="stack") + facet_wrap(~iso3, scales="free_y") + theme_report() + scale_fill_brewer('Outcome', type="qual", palette=6) + scale_x_continuous("") + scale_y_continuous("Percent of cohort") + coord_cartesian(ylim=c(0,100)) + guides(fill = guide_legend(reverse = TRUE))


ggplot(trc, aes(year, value, fill=variable)) + geom_area(stat="identity", position="stack") + facet_wrap(~iso3, scales="fixed") + theme_report() + scale_fill_brewer('Outcome', type="qual", palette=6) + scale_x_continuous("", breaks=c(2000,2005,2010)) + scale_y_continuous("Percent of cohort") + coord_cartesian() + guides(fill = guide_legend(reverse = TRUE))
...
ggplot(thb, aes(year, value)) + geom_line(size=1) + facet_grid(variable~iso3, scales="free") + theme_report() + labs(y="Percent", x="") + scale_x_continuous(breaks=c(2005, 2010))
...
ga <- gc %>%
  filter(iso3 %in% lic) %>% 
  select(iso3, year, gdp=NY.GDP.PCAP.KD, mort=e_mort_exc_tbhiv_100k, mort.lo=e_mort_exc_tbhiv_100k_lo, mort.hi=e_mort_exc_tbhiv_100k_hi, c_newinc_100k)


ggplot(ga, aes(gdp, c_newinc_100k, color=iso3)) + geom_path(size=1, arrow=arrow(length=unit(0.3, "cm"))) + theme_report() + scale_x_log10("GDP per capita (constant 2000 US$)") + scale_y_log10("Estimated TB mortality per 100 000 per year") + scale_color_brewer("", type="qual", palette=6) + geom_text(data=subset(ga, year==yr), aes(label=iso3), hjust=1, vjust=1.5) + theme(legend.position="none")


, limits=c(300, 4000), breaks=c(300, 1e3, 2e3, 3e3, 4e3)
, limits=c(1, 200), breaks=c(1, 10, 50, 100, 200)
