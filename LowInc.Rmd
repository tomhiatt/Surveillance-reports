---
title: Epidemiological Situation of Tuberculosis in countries of the Western Pacific
  with a low or intermediate disease burden using `r yr <- 2014; yr` case notification
  data
...
if(FALSE){
  ...
  gb <- WDI(indicator='NY.GDP.PCAP.KD', start=2000, end=yr, extra=FALSE)
  oba <- WDI(indicator='SM.POP.TOTL.ZS', start=1980, end=yr, extra=FALSE)
...
tbli <- tb %>% filter(iso3 %in% lic) %>% arrange(country) 
  
   save(esta, tb, tbli, p, gb, oba, file="./laddata.Rdata")
...
tbb <- tbli %>% filter(year==yr) %>% mutate(area=country, total="Total") %>% select(area, total, c_notified, c_newinc, new_labconf, new_clindx, new_ep, ret_rel_labconf, ret_rel_clindx, ret_rel_ep, ret_nrel)
...
spa <- tbli %>% filter(year==yr) %>% select(c_newinc_100k, c_newinc, iso3, e_pop_num)
...
# tbli %>% filter(year==yr) %>% ggplot(aes(c_newinc_100k, c_newinc, group=iso3)) + geom_text(aes(label=iso3)) + theme_report() + theme(legend.position="none") + labs(x="New and relapse cases per 100 000", y="Total new and relapse cases") + xlim(c(0,100))
...
trg <- tbli %>% filter(year %in% 1980:yr) %>% select(year, c_newinc_100k, iso3) %>% mutate(orphs=orphaned.dots(c_newinc_100k))

# Get exponential prediction
trh <- NULL
for(cnty in lic){
  fitbegin <- 2005
  temp1 <- trg %>% filter(iso3==cnty) %>% mutate(c_newinc_100k.part = ifelse(year<fitbegin, NA, c_newinc_100k))
  emod <- lm(log(c_newinc_100k.part)~year, temp1)
  temp1$fit.y <- c(rep(NA, fitbegin-1980), exp(predict(emod,list(year=fitbegin:yr))) )
  temp1$coef <- sprintf("%1.1f%%", round(coef(emod)[2] * 100, 1))
  trh <- rbind(trh, temp1)
}


f.trend <- trh %>% ggplot(aes(year, c_newinc_100k, ymin=0)) + geom_line(size=1) + geom_point(data=subset(trg, orphs)) + theme_report() + labs(x="", y="Total new and relapse cases per 100 000 per year") + facet_wrap(~iso3, scales="free_y") + geom_line(aes(y=fit.y), color=wpsar_colors[1], linetype="solid", size=0.75) + geom_text(aes(label=coef), data = subset(trh, year==yr-3), size=3, vjust=2.5, color=wpsar_colors[1]) #+ geom_smooth(method="lm", se=FALSE) #+ scale_y_log10()

write.csv(trg, file=paste0(pasteLabel("./figure_data/table", figCount, "f.trend", insLink=FALSE, sepper=""), ".csv"), row.names=FALSE, na="")

# For paragraph
# trg %>% filter(year %in% c(2000, 2013)) %>% spread(year, c_newinc_100k) %>% mutate(decline=a.rate(`2000`, `2013`, 13)) %>% arrange((decline))
...
aga <- merge(subset(tbli, select=c("iso3", "country", "year", "g_whoregion", "g_hbc22", "newrel_m04", "newrel_m514", "newrel_m014", "newrel_m1524", "newrel_m2534", "newrel_m3544", "newrel_m4554", "newrel_m5564", "newrel_m65", "newrel_mu", "newrel_f04", "newrel_f514", "newrel_f014", "newrel_f1524",  "newrel_f2534", "newrel_f3544", "newrel_f4554", "newrel_f5564", "newrel_f65", "newrel_fu",
...
# Subset for WPRO for years of interest
age <- subset(agd, (iso3 %in% lic | area=="LIC") & age!="u" & !age %in% c("04", "514")) %>% filter(year %in% 1995:yr)
...
p1 <- ggplot(subset(age, year %in% 1995:yr & iso3 %in% lic[1:3]), aes(year, newrel_100k, color=age)) + geom_line(size=1) + facet_grid(iso3~sex, scales='free_y') + theme_report() + scale_x_continuous("", breaks = pretty_breaks()) + scale_color_brewer("Age group", type="seq", palette=4)
...
f.agesex <- ggplot(subset(age, year %in% 1995:yr & iso3 %in% lic[1:9]), aes(year, newrel_100k, color=age, width=trnd.brk)) + geom_line(size=1) + facet_wrap(~iso3, scales='free_y', nrow=3) + theme_report() + scale_x_continuous("", breaks = pretty_breaks()) + scale_color_manual("Age group", values=wpsar_colors) + scale_y_continuous("New and relapse cases per 100 000 per year") + guides(color = guide_legend(reverse = TRUE)) + theme(legend.text.align=1)
...
f.migrant.trend <- ggplot(obb, aes(year, migtb_pct, ymin=0)) + geom_point(aes(size=mig)) + geom_line() + theme_report() + scale_x_continuous("", limits=c(2007,yr)) + scale_y_continuous("Foreign-born TB cases (% of total notified)") + facet_wrap(~iso3, scales = "free_y") + scale_size("International migrant stock 2010 (% of population)") + theme(legend.position="bottom")

obq <- obb %>%
  mutate(mig=ifelse(year==2010, mig, NA), migtb_pct=ifelse(year==2014, migtb_pct, NA)) %>%
  group_by(iso3) %>%
  summarize(mig=sum0(mig), migtb_pct=sum0(migtb_pct)) %>%
ggplot(aes(mig, migtb_pct)) + geom_point(aes(color=iso3)) + geom_line(aes(x,y), data.frame(x=c(0,60), y=c(0,60)), alpha=0.2) + theme_report() + scale_x_continuous("International migrant stock 2010 (% of population)") + scale_y_continuous("Foreign-born TB cases 2014 (% of total notified)") + scale_color_manual(values=rep(wpsar_colors, 2))

f.migrant <- direct.label(obq)


obq <- obb %>%
  mutate(mig=ifelse(year==2010, mig, NA), c_newinc_100k=ifelse(year==2014, c_newinc_100k, NA)) %>%
  group_by(iso3) %>%
...
# For stack overflow
# The stack area doesn't show gaps.
require(dplyr)
require(ggplot2)
...
n34 <- tbli %>% mutate(idef=signif((c_newinc-newrel_coh)/c_newinc * 100, 2)) %>% filter(year==yr-1, iso3 %in% lic, idef > 2) %>% select(country, idef) %>% arrange(desc(idef)) 
...
tha <- subset(tb, year %in% 2005:yr & g_whoregion=="WPR" & iso3 %in% lic, select=c("country", "year", "iso3", "g_hbc22", "c_notified", "hivtest", "hivtest_pos", "hiv_cpt", "hiv_art", "hivtest_pct_numerator", "hivtest_pct_denominator", "hivtest_pos_pct_numerator", "hivtest_pos_pct_denominator", "hiv_cpt_pct_numerator", "hiv_cpt_pct_denominator", "hiv_art_pct_numerator", "hiv_art_pct_denominator"))
...
foreign_born_pct <- tb %>% filter(year==2014) %>% filter(iso3 %in% lic) %>% summarize(mig=sum0(c(notif_foreign)), c_notified=sum0(c_notified))
```


**Introduction**: The End TB Strategy, adopted by the World Health Assembly in 2014, expands the scope of TB control beyond countries with a high burden of TB to also include countries and areas with a low and intermediate burden of the disease. Analysis of TB data from these countries provides an opportuntity to assess progress and inform future directions.

**Methods**: Case notifications, treatment outcomes and TB/HIV progress from nine low- and intermediate-burden countries and areas in the Region (LICs) were analysed---Australia; Brunei Darussalam; China, Hong Kong SAR; China, Macao SAR; Japan; Malaysia; New Zealand; Republic of Korea and Singapore. These countries and areas make up 13.5% of the regional population.

**Results**: In 2014, LICs in the Region reported 97 thousand people with TB disease accounting for 7.02% of the regional burden. Case notification rates have mostly trended downward since 1980. Most LICs show a consecutive decrease in TB trends from older to younger age groups with children under 15 substantially lower than the next eldest age group. The proportion foreign-born amongst TB cases and that of the general population ranges greatly amongst the LICs. All LICs besides Australia and New Zealand have the majority of TB cases coming from the domestic-born population. For the 2013 cohort, the proportion of patients successfully treated was highest in China, Macao SAR (88.3%) and lowest in Japan (54.1%). Patients not evaluated was the most common unfavourable treatment outcome, followed by death and lost to follow-up. HIV testing among TB patients is high or increasing in all LICs except Japan. Co-trimoxazole preventive therapy (CPT) and antiretroviral therapy (ART) coverage is largely unreported and low in most LICs.

**Discussion**: The TB burden found in these nine LICs, while comparatively small, still represents nearly 100 000 cases in the Region. The epidemiological situation amongst the LICs is varied and suggests diverse strategies for controlling TB. Migration continues to be a key issue for TB control, although less than 10% of TB cases in LICs are foreign-born. Treatment success is still low in most LICs, which may be attributable to complications surrounding TB treatment for the elderly. Patients lost to follow-up is still a problem, especially in Japan, Malaysia and the Republic of Korea. HIV testing is over 50% in most LICs and rising. The proportion of TB/HIV cases found is low indicating a good level of testing coverage and prioritization of patients tested. CPT and ART coverage, however, should receive more attention as proportions are well below 100%. Within inherent limitations, TB surveillance data are an important source of programmatic and epidemiological information. Careful interpretation of these findings can provide useful insight for programmatic decision-making.
...
The End TB Strategy, adopted by the World Health Assembly in 2014,[@WorldHealthOrganization2014a] expands the scope of TB control beyond countries with a high burden of TB to also include countries and areas with a low and intermediate burden of the disease. The challenges faced by these countries are diverse and often require innovative solutions. WHO released a framework towards TB elimination in countries that have a low burden of disease,[@WorldHealthOrganization2014] which requires concerted efforts on marginalized and vulnerable populations. In the WHO Western Pacific Region, significant progress has been made in the past few decades in reducing the burden of TB including among higher-income countries with a lower disease burden. Analysis of TB data from these countries provides an opportuntity to assess progress and inform future directions.

Methods
--------------------------------------------------------
```{r methods, echo=FALSE}
m1 <- lister(unique(tbli$country), ";")
m2 <- signif(tbli %>% filter(year==yr) %>% summarize(pop=sum(e_pop_num)) / tb %>% filter(year==yr, g_whoregion=="WPR") %>% summarize(pop=sum(e_pop_num)) * 100,3)
m2b <- signif(tbli %>% filter(year==yr) %>% summarize(notif=sum0(c_notified)) / tb %>% filter(year==yr, g_whoregion=="WPR") %>% summarize(pop=sum0(c_notified)) * 100,3)

m3 <- tbli %>% filter(year %in% 2012) %>% mutate(nd=new_su / c_newinc * 100) %>% group_by(country) %>% summarize(ndo=round(max(nd),0)) %>% arrange(desc(ndo)) %>% head(3) %>% as.data.frame() 
...
m6 <- tbli %>% filter(year %in% 2012, g_whoregion=="WPR") %>% group_by(year) %>% summarize(new_su=sum0(new_su), c_newinc=sum0(c_newinc), nd=signif(new_su/c_newinc  * 100,2)) %>% arrange(desc(nd)) %>% head(1) %>% select(nd) 
...
In `r yr`, LICs in the Region reported `r n1` people with TB disease (`r I(pasteLabel("Table", tableCount, "t.notif"))`). This accounts for `r m2b`% of the regional burden. New and relapse cases (new episodes of TB) accounted for 96.2% of those notified. Pulmonary new and relapse cases were bacteriologically confirmed in 72% of LIC cases, ranging from 62% in Singapore to 96% in Brunei Darussalam. The proportion of extrapulmonary cases tend to form three groups, high proportions of over 35% in Australia and New Zealand, low proportions in China, Macao SAR, Malaysia and Singapore with around 15%, and the remaining LICs around 20%. Relapse cases comprise 8.7% of cases in these countries with the highest in the Republic of Korea at 13%. Other retreatment cases are generally low with the highest proportions in Malaysia (2.7%) and the Republic of Korea (6.7%). All proportions presented are higher in LICs combined total than the WPR average.

`r I(pasteLabel("Figure", figCount, "f.spread"))` displays the differences in TB burden between LICs. New and relapse TB notification rates are mapped to a horizontal log scale to better show the natural progression as disease burden decreases. The vertical scale shows the actual number of cases per year. Japan, with the largest population, sits in the middle of this spread with an existing burden of nearly 20 thousand cases per year. Australia and New Zealand have comparatively lower burdens of TB in terms of proportion and absolute numbers. Brunei Darussalum, China, Hong Kong SAR, China, Macao SAR, and Singapore have higher proportions of TB, but a comparatively lower absolute burden due to smaller population sizes. With similar, but slightly higher proportions, Malaysia and the Republic of Korea have much higher case loads due to their larger populations with over 24 and 40 thousand cases, respectively. 

Case notification rates have mostly trended downward since 1980, with the exception of rates in Malaysia and Singapore (`r I(pasteLabel("Figure", figCount, "f.trend"))`). Trends have tapered off in recent years in Australia, Brunei Darussalam, the Republic of Korea, and China, Macao SAR. The trend modeled from 2005 to 2014 shows the greatest annual rate of decline to be in Japan (3.7%).
...
prop.new.licall <- tb %>% filter(year==2014, g_whoregion=="WPR") %>% mutate(lic=iso3 %in% lic) %>% group_by(lic) %>% summarize(prop.new=sum0(c_newinc)/sum0(c_notified) * 100)

  
```


The TB burden found in these nine LICs, while comparatively small, still represents nearly 100 000 cases in the Region. As these countries are wealthier than the regional average, the higher levels of bacteriological confirmation may be attributed to the greater availability of more sensitive diagnostics. As noted above, the proportion of cases which did not receive confirmatory testing is unknown, but likely not insignificant. The proportion of extrapulmonary at two and a half times the regional average may also be a result of more sophisticated diagnostics and increased clinical ability to detect these cases. 

The epidemiological situation amongst the LICs is varied and suggests diverse strategies for controlling TB. Australia and New Zealand meet the criteria of less than 10 cases per 100 000 identified for targeting elimination.[@WorldHealthOrganization2014] Similar to other countries with a low burden of TB, the challenge of providing effective TB services for migrants should be prioritized if elimination is to be realized.[@WorldHealthOrganizationRegionalOfficefortheWesternPacific2016] 

Where active transmission has been interrupted, TB disease is only a result of reactivation of previous infection. `r I(pasteLabel("Figure", figCount, "f.agesex"))` presents TB trends in age groups and reveals diverse epidemiological situations. Australia and New Zealand have the highest proportions of foreign-born cases (`r I(pasteLabel("Figure", figCount, "f.migrant"))`) and show the highest rates in the 15--44 age groups. China, Hong Kong SAR and Japan show general downward trends in nearly all age groups indicating some transmission control despite aging populations. Malaysia shows increasing trends in nearly all age groups giving evidence toward ongoing transmission and a growing epidemic. In the Republic of Korea, TB in the older groups is increasing while decreasing in the younger groups with minimal foreign-born cases. This situation warrants further investigation.

Migration continues to be a key issue for TB control, although with the exception of Australia and New Zealand, the majority of TB cases are not in migrant populations (`r I(pasteLabel("Figure", figCount, "f.migrant"))`). As the proportion of infected individuals amongst the elderly decreases in Singapore and Brunei Darussalam, effective TB services for migrant populations will become more important. In Japan, Malaysia and the Republic of Korea, the vast majority of TB cases are in the native-born population. China, Hong Kong SAR and China, Macao SAR have lower than expected migration amongst their TB cases compared to Australia and New Zealand. This may be due to the predominate source countries for migrants and due to migrant screening policy differences.

Treatment failure is considerably low in recent years and the number of unevaluated cases is gradually reducing (`r I(pasteLabel("Figure", figCount, "f.txout"))`). Treatment success is still low in most LICs, which may be attributable to complications surrounding TB treatment for the elderly. Patients lost to follow-up is still a problem, especially in Japan, Malaysia and the Republic of Korea. HIV testing is mostly over 50% and rising (`r I(pasteLabel("Figure", figCount, "f.tbhiv"))`). The proportion of TB/HIV cases found is low indicating a good level of testing coverage and prioritization of patients tested. CPT and ART coverage, however, should receive more attention as proportions are well below 100%.

This analysis provides a snapshot of the epidemiological and programmatic situation of TB in these select countries and areas based on the case notification data in `r yr`. As for any disease surveillance system, the analysis of surveillance data has inherent limitations. TB is a notifiable disease in all LICs, however adherence to notification protocols and fluctuation in government public health performance can strongly influence observed epidemiological trends. A careful assessment is needed of programmatic progress in the country and the quality of surveillance data when interpreting these findings.
...
print(tbm, type="html", include.rownames=FALSE, include.colnames=FALSE, html.table.attributes="border=0 rules=rows width=700 cellpadding=15", add.to.row=list(pos=list(0, nrow(tbm)), command=c(
  ...
  <TD>EXRAPULMONARY AMONG TOTAL (%)</TD> 
  <TD style='border-right: solid 1px black;'>RELAPSE AMONG TOTAL (%)</TD> 
...
<a id="f.trend"></a> 
**`r I(pasteLabel("Figure", figCount, "f.trend", insLink=FALSE))`. Tuberculosis case notification rate (all forms and new laboratory-confirmed) per 100 000 population in select countries of the Western Pacific Region, `r min(trg$year)`--`r max(trg$year)`** 

```{r f.trend, fig.height=6, warning=FALSE}
f.trend
```

Green trendlines and numbers in the plot area indicate the exponential fit and annual change for the last 10 years.
...
<a id="f.agesex"></a> 
**`r I(pasteLabel("Figure", figCount, "f.agesex", insLink=FALSE))`. Trend of age group-specific notification rates (per 100 000 population) of new and relapse^a^ tuberculosis cases in select countries of the Western Pacific Region, `r min(age$year)`--`r max(age$year)`**  
...
<a id="f.migrant"></a> 
**`r I(pasteLabel("Figure", figCount, "f.migrant", insLink=FALSE))`. Foreign-born TB cases, `r yr`, and international migrant stock, 2010, in nine countries of the Western Pacific Region**  
```{r f.migrant, fig.height=5, warning=FALSE}
f.migrant
```

The diagonal line indicates equal proportions on horizontal and vertical scales.
International migrant stock data are from World Development Indicators, World Bank.
...
<a id="f.txout"></a> 
**`r I(pasteLabel("Figure", figCount, "f.txout", insLink=FALSE))`. Trend of treatment outcome expressed as a proportion among new laboratory-confirmed cases in the Western Pacific Region, `r min(trc$year)`--`r max(trc$year)`**  
...
<a id="f.tbhiv"></a> 
**`r I(pasteLabel("Figure", figCount, "f.tbhiv", insLink=FALSE))`. Progress in TB/HIV activities in select countries in the Western Pacific Region, `r min(thb$year)`--`r max(thb$year)`**  
...
# file.copy("D:/Users/hiattt/Dropbox/Code/Surveillance reports/LowInc.html", "D:/Users/hiattt/Dropbox/STB-WPRO 2016/WPSAR articles/LIburden.html", overwrite = TRUE)
```
