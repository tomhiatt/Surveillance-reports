...
<sup>&dagger;</sup> Stop TB and Leprosy Elimination, Division of Combating Communicable Diseases, World Health Organization Regional Office for the Western Pacific, Manila, the Philippines.
...
oe <- subset(od, select=c("e_new_mdr_pct", "e_new_mdr_pct_range", "e_ret_mdr_pct", "e_ret_mdr_pct_range", "e_mdr_num", "e_mdr_num_range", "mdr_new_pct", "mdr_ret_pct", "mdr", "rapid_dx_dr_r", "conf_mdr_tx", "mdr_tx"))
...
##### `r I(pasteLabel("Table", tableCount, "t-drestnotif", insLink=FALSE))`.  		Table or map (survey/ surveillance, single/several data point, 				national/ sub national)
```{r t-drestnotif, results='asis'}
             
# Make that table
htmlTable(oe, caption = "", rowlabel = "", cgroup = rbind(c("Estimates", "Reported data", rep(NA,7)), c("% MDR-TB among new", "% MDR-TB among ret", "MDR-TB cases ___________________", "% MDR-TB among new", "% MDR-TB among ret", "MDR-TB cases", "RR cases<sup>*</sup>", "Confirmed MDR-TB on treatment", "Total MDR-TB on treatment")), n.cgroup = rbind(c(6,6, rep(NA,7)), c(2,2,2,1,1,1,1,1,1)), align=c(rep(c('r','l'),3), rep("c",6) ), ctable = TRUE, tfoot = "<sup>*</sup> Additional Rifampicine resistant cases detected by Xpert.", headings = NA )
...
# Make that table
htmlTable(nbf, caption = "", rowlabel = "", cgroup = c("Cases with culture or LPA results", "% with DST results", "% with resistance to H only", "% with resistance to R only", "% MDR-TB<sup>*</sup>", "Additional cases confirmed by Xpert", "% Xpert cases MDR-TB<sup>*</sup>"), n.cgroup = c(3,3,3,3,3,3,3), ctable = TRUE, tfoot = "<sup>*</sup> Resistant to H and R.")
```
