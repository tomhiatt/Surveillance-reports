...
rnmap.df$notif2<-rnmap.df$notif/10
...
theme_set(theme_classic())
rgtm<-ggplot(wmap, aes(long,lat,group=group))+geom_polygon()
rgtm<-rgtm+geom_polygon(data=rnmap.df,aes(long,lat, group=group, fill=notif2))+scale_fill_gradient("New and relapse\ncases per\n1 000 000\npopulation, 2012",low="#ff0000",high="#4c0000", space="Lab")+xlim(c(73.55,157))+ylim(c(-11,53))+coord_map()
...
jpeg(file=file.path(figurefolder, paste("regionalns3",".jpeg", sep="")),w=2115,h=2000,res=300)
print(rgtm)
dev.off()
